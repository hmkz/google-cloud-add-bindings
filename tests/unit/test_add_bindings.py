...
class TestAddBindings:
    ...
    def invalid_csv_path(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".csv") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["user_email", "project_id", "asset_type", "role"]
            )  # asset_nameカラムがない
            writer.writerow(
                [
                    "user1@example.com",
    ...
    def missing_resource_name_csv_path(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".csv") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["user_email", "project_id", "resource_type", "resource_name", "role"]
            )
            writer.writerow(
                ["user1@example.com", "project-1", "project", "", "roles/viewer"]
            )
            writer.writerow(
                [
                    "user1@example.com",
    ...
    def test_process_csv(self, mock_iam_manager_class, valid_csv_path):
        ...
        assert (
            calls[0][1]["asset_name"]
            == "//cloudresourcemanager.googleapis.com/projects/project-1"
        )
        assert (
            calls[0][1]["asset_type"] == "cloudresourcemanager.googleapis.com/Project"
        )
