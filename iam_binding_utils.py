...
class IAMBindingManager:
    ...
    def load_config(self, config_file: str):
        ...
        try:
            ...
            logger.info(f"設定ファイルから {len(asset_types)} 個のアセットタイプを読み込みました: {config_file}")
            
        except Exception as e:
            logger.error(f"設定ファイルの読み込みエラー: {e}")
            raise ValueError(f"設定ファイルの読み込みに失敗しました: {e}")
    
    def register_asset_type(self, 
                          asset_type: str, 
                          service_name: str, 
                          version: str, 
                          method: str,
    ...
    def parse_asset_name(self, asset_name: str, asset_type: str) -> Dict[str, str]:
        ...
        if not match:
            raise ValueError(f"無効なアセットネーム形式: {asset_name} (タイプ: {asset_type})")
    ...
    def get_current_policy(self, asset_name: str, asset_type: str, project_id: str) -> Dict:
        """
        ...
        client = self.get_client(resource_config['service_name'], resource_config['version'])
        ...
        try:
            # 組み込みアセットタイプの処理
            if asset_type == 'cloudresourcemanager.googleapis.com/Project':
                response = client.projects().getIamPolicy(
                    resource=resource_info['project_id'],
                    body={}
                ).execute()
                return response
            
            elif asset_type == 'storage.googleapis.com/Bucket':
                response = client.buckets().getIamPolicy(
            ...
            # カスタムアセットタイプの汎用処理
                ...
                if hasattr(self, method_name):
                    ...
                    logger.warning(f"カスタムポリシー取得メソッドが未定義: {method_name}")
                    raise ValueError(f"アセットタイプ {asset_type} のポリシー取得方法が実装されていません")
    ...
    def add_binding(self, 
                   ...
                   asset_type: str,
                   project_id: str,
                   dry_run: bool = False) -> Tuple[bool, Any]:
        """
        IAMバインディングを追加します
        ...
        try:
            ...
            # ドライランの場合はログ出力のみ
            if dry_run:
                logger.info(f"[ドライラン] ユーザー {user_email} にロール {role} を {asset_name} に付与します")
                return True, None
            
            # 組み込みアセットタイプの処理
            if asset_type == 'cloudresourcemanager.googleapis.com/Project':
                return self._add_binding_to_project(client, user_email, role, resource_info, current_policy)
            
            elif asset_type == 'storage.googleapis.com/Bucket':
                return self._add_binding_to_bucket(client, user_email, role, resource_info, current_policy)
            
            elif asset_type == 'bigquery.googleapis.com/Dataset':
                return self._add_binding_to_dataset(client, user_email, role, resource_info, current_policy)
                
            elif asset_type == 'bigquery.googleapis.com/Table':
                return self._add_binding_to_table(client, user_email, role, resource_info, current_policy)
                
            elif asset_type == 'bigquery.googleapis.com/Model':
                return self._add_binding_to_model(client, user_email, role, resource_info, current_policy)
                
            elif asset_type == 'bigquery.googleapis.com/Routine':
                return self._add_binding_to_routine(client, user_email, role, resource_info, current_policy)
            ...
            # カスタムアセットタイプの汎用処理
                ...
                if hasattr(self, method_name):
                    custom_method = getattr(self, method_name)
                    return custom_method(client, user_email, role, resource_info, current_policy)
                else:
                    logger.warning(f"カスタムバインディング追加メソッドが未定義: {method_name}")
                    raise ValueError(f"アセットタイプ {asset_type} のバインディング追加方法が実装されていません")
    ...
    def _add_binding_to_project(self, client, user_email, role, resource_info, current_policy):
        ...
        # ポリシーを更新
        response = client.projects().setIamPolicy(
            resource=resource_info['project_id'],
            body={'policy': current_policy}
        ).execute()
        
        logger.info(f"ユーザー {user_email} にロール {role} をプロジェクト {resource_info['project_id']} に付与しました")
        return True, response
    
    def _add_binding_to_bucket(self, client, user_email, role, resource_info, current_policy):
        ...
        # ポリシーを更新
        response = client.buckets().setIamPolicy(
            bucket=resource_info['bucket_name'],
            body=current_policy
        ).execute()
        
        logger.info(f"ユーザー {user_email} にロール {role} をバケット {resource_info['bucket_name']} に付与しました")
        return True, response
    
    def _add_binding_to_dataset(self, client, user_email, role, resource_info, current_policy):
        """
        ...
        try:
            ...
            for access in new_access:
                if access.get('userByEmail') == user_email and access.get('role') == role_name:
            ...
            logger.info(f"データセットにバインディングを追加しました: {project_id}, {dataset_id}, {user_email}, {role}")
            return True, response
    ...
    def _add_binding_to_table(self, client, user_email, role, resource_info, current_policy):
        """
        ...
        try:
            ...
            logger.info(f"テーブルにバインディングを追加しました: {project_id}, {dataset_id}, {table_id}, {user_email}, {role}")
            return True, response
    ...
    def _add_binding_to_model(self, client, user_email, role, resource_info, current_policy):
        """
        ...
        try:
            ...
            logger.info(f"モデルにバインディングを追加しました: {project_id}, {dataset_id}, {model_id}, {user_email}, {role}")
            return True, response
    ...
    def _add_binding_to_routine(self, client, user_email, role, resource_info, current_policy):
        """
        ...
        try:
            ...
            logger.info(f"ルーティンにバインディングを追加しました: {project_id}, {dataset_id}, {routine_id}, {user_email}, {role}")
            return True, response
