...
class TestIAMBindingManager:
    ...
    def test_init_without_credentials(self):
        ...
        assert 'cloudresourcemanager.googleapis.com/Project' in manager.asset_type_mapping
    ...
    def test_load_config_yaml(self, custom_config_yaml):
        ...
        # Subscriptionの設定を確認
        subscription_config = manager.asset_type_mapping['pubsub.googleapis.com/Subscription']
    ...
    def test_update_asset_pattern(self, iam_manager):
        """アセットパターンを更新するテスト"""
        # 既存のパターンを取得
        old_pattern = iam_manager.get_asset_pattern_for_type('storage.googleapis.com/Bucket')
        ...
        # 更新されたことを確認
        updated_pattern = iam_manager.get_asset_pattern_for_type('storage.googleapis.com/Bucket')
    ...
    def test_get_current_policy_project(self, mock_get_client, iam_manager):
        ...
        mock_projects.getIamPolicy.assert_called_once_with(resource='test-project', body={})
    ...
    def test_get_current_policy_dataset(self, mock_get_client, iam_manager):
        ...
        mock_datasets.get.assert_called_once_with(projectId='test-project', datasetId='test_dataset')
        assert policy == []
    
    @patch('iam_binding_utils.IAMBindingManager.get_current_policy')
    @patch('iam_binding_utils.IAMBindingManager.get_client')
    def test_add_binding_project(self, mock_get_client, mock_get_current_policy, iam_manager):
    ...
    @patch('iam_binding_utils.IAMBindingManager.get_current_policy')
    @patch('iam_binding_utils.IAMBindingManager.get_client')
    def test_add_binding_bucket(self, mock_get_client, mock_get_current_policy, iam_manager):
    ...
    @patch('iam_binding_utils.IAMBindingManager.get_current_policy')
    @patch('iam_binding_utils.IAMBindingManager.get_client')
    def test_add_binding_dataset(self, mock_get_client, mock_get_current_policy, iam_manager):
        ...
        assert len(access) == 1
        assert access[0]['role'] == 'dataViewer'  # roles/bigquery.dataViewer -> dataViewer
        assert access[0]['userByEmail'] == 'user@example.com'
    
    def test_dry_run(self, iam_manager):
        """ドライランモードのテスト"""
        with patch('iam_binding_utils.IAMBindingManager.get_current_policy') as mock_get_policy:
