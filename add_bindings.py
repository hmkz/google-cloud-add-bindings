...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Google Cloud IAMバインディングをCSVから追加"
    )

    parser.add_argument("--csv-file", required=True, help="処理するCSVファイルのパス")

    parser.add_argument(
        "--credentials", help="Google Cloudサービスアカウントキーファイルのパス"
    )
    ...
    parser.add_argument(
        "--config-file", help="アセットタイプ設定ファイルのパス (.json, .yaml, or .yml)"
    )

    parser.add_argument(
        "--export-config", help="現在のアセットタイプ設定をエクスポートするファイルパス"
    )
...
def process_csv(
    csv_path: str, iam_manager: IAMBindingManager, dry_run: bool = False
) -> Dict:
    """
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            try:
                logger.info(
                    f"処理中: {user_email}, {project_id}, {asset_name}, {asset_type}, {role}"
                )
        ...
        results["errors"].append({"row": 0, "data": {}, "error": str(e)})
        return results
...
def main():
    ...
    try:
        ...
        iam_manager = IAMBindingManager(
            credentials_path=args.credentials, config_file=args.config_file
        )
        ...
        if args.export_config:
            ...
            logger.info(
                f"アセットタイプ設定を {args.export_config} にエクスポートしました"
            )
            return
        ...
        # 結果の出力
        logger.info(
            f"処理完了: 合計={results['total']}, 成功={results['success']}, 失敗={results['failed']}"
        )
